...
\begin{document}
    ...
    Tiểu luận tổng hợp về lý thuyết và ứng dụng của xích Markov, lý thuyết phân nhánh, xích Markov Monte Carlo, quá trình quyết định Markov và xích Markov ẩn.
    ...
    \begin{algorithm}
        \DontPrintSemicolon
        \KwIn{Tập các trạng thái $\mathscr{E}$, tập các tín hiệu $\mathscr{S}$, dãy tín hiệu quan sát được $\mathbf{S}^n=(s_1, s_2, \dots, s_n)$, ma trận chuyển trạng thái $\mathbb{P}=(P_{ij})_{i,j \in \mathscr{E}}$, ma trận xác suất bức xạ $\mathbb{G}=(G_{sj})_{s \in \mathscr{S}, j \in \mathscr{E}}=(p(s \vert j))_{s \in \mathscr{S}, j \in \mathscr{E}}$, phân bố ban đầu $p_i = P(X_1 = i), i \in \mathscr{E}$}

        \ForEach{$j \in \mathscr{E}$}{
            $V_1(j) \gets P \lbrace X_1 = j, S_1 = s_1 \rbrace = p_j p(s_1 \vert j)$\;
        }

        \ForEach{$k \gets 1$ \KwSty{to} $n$}{
            \ForEach{$j \in \mathscr{E}$}{
                $V_k (j) \gets p(s_k \vert j) \max_i P_{ij} V_{k-1}(i)$\;
            }
        }

        $j_n \gets \underset{j}{\mathrm{argmax}} \thickspace V_n(j)$ \tcp*{Tìm trạng thái tại bước cuối cùng}

        \ForEach{$k \gets n-1$ \KwTo{to} 1}{
            \tcp*[l]{Tính ngược trở lại các trạng thái tại các bước phía trước đến bước đầu tiên}
            $j_{k} \gets \underset{j}{\mathrm{argmax}} \thickspace p(s_{k+1} \vert j_{k+1}) P_{j, j_{k+1}}V_{k} (j)$\;
        }

        \Return{$(j_1, \dots, j_n)$}\;
        \caption{Thuật toán Viterbi}
        \label{alg:Viterbi-algorithm}
    \end{algorithm}
